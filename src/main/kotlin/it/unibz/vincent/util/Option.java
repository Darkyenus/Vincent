...
public final class Option {
	...
	public Option(char shortName, String longName, String description, Boolean argument, String argumentDescription, OptionHandler handler) {
	...
	public Option(char shortName, String longName, String description, OptionHandler handler) {
		this.shortName = shortName;
		this.longName = longName;
		this.description = description;
	...
	/** Print option help. */
	public static void printLaunchHelp(Option[] options) {
		// https://www.gnu.org/prep/standards/html_node/_002d_002dhelp.html
		final PrintStream err = System.err;
		err.println("Research Cloud Server");

		// One StringBuilder per line to do nice alignment
	...
	public static String[] parseOptions(String[] args, Option[] options) {
		...
		while (argsIndex < args.length) {
			...
			if ("--".equals(arg)) {
				...
				if (option == null) {
					err.println(APP_NAME +": unrecognized option '--"+optName+"'");
					return null;
				}

				if (option.argument == Boolean.FALSE && equalsIndex >= 0) {
					err.println(APP_NAME +": option '--"+optName+"' doesn't allow an argument");
					return null;
				}

				if (option.argument == Boolean.TRUE && equalsIndex < 0) {
					err.println(APP_NAME +": option '--"+optName+"' requires an argument");
					return null;
				}
